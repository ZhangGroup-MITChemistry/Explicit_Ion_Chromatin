...
class System:
    ...
    def write_pdb(self):
        chains = ['A', 'B', 'C', 'D', 'E', 'F']
        fptr = open("in00_conf.pdb",'w')
        fptr.write('CRYST1%9.3f%9.3f%9.3f%7.2f%7.2f%7.2f %11s%4d\n'%(0,0,0, 90, 90, 90, 'P', 1))
        ...
        for i in range(self.number_molecules()):
            for j in range(self.molecules[i].number_beads()):
                bead = self.molecules[i].beads[j]
                          ...
                          #'ATOM      1  S   THY Y   1     113.339 -22.757 109.841  0.00  0.00      DNA1 S'
                fptr.write('ATOM  %5d %2s%1s%1s%1s%3s %1s%4d%1s   %8.3f%8.3f%8.3f%6.2f%6.2f      %4s%2s%2s\n'%(j+nbeads+1, '', bead.beadtype,'','', bead.residuetype, chains[i], bead.residuenumber, '', bead.x, bead.y, bead.z, 0.0, 0.0, 'DNA', 'S', ''))
        ...
        for i in range(self.number_molecules()):
            for j in range(self.molecules[i].number_beads()):
                ...
                if j == self.molecules[i].number_beads()-1:
                    shift = 8
                fptr.write("\t%ld\t%ld\t%ld\t%ld\t%lf\t%lf\t%lf\t%lf\n" % (j+nbeads+1, i+1, bead.residuenumber, siteNumber[bead.beadtype]+shift, bead.charge,bead.x, bead.y, bead.z))
            nbeads = nbeads + self.molecules[i].number_beads()
        fptr.close()
