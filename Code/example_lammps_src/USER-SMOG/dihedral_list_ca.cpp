...
DihedralListCA::DihedralListCA(LAMMPS *lmp) : Dihedral(lmp) {}

/* ---------------------------------------------------------------------- */

DihedralListCA::~DihedralListCA()
{
  if (allocated) {
...
void DihedralListCA::compute(int eflag, int vflag)
{
  ...
  double cosphi, aphi, dtor, dtor3, expt;
  ...
  for (n = 0; n < ndihedrallist; n++) {
    ...
    dtor3 = (aphi - phi[type])*3; // Change made to get things to "work"
    //dtor = aphi + phi[type];
    ...
    edihedral = 0.0;
    // Periodic torsion
    df = -kperiodic[type] * ( sin(dtor) + 1.5*sin(dtor3) );
    if (eflag) edihedral = kperiodic[type] * ( (1-cos(dtor)) + 0.5*(1-cos(dtor3)) );
...
void DihedralListCA::settings(int narg, char **arg)
{
...
void DihedralListCA::coeff(int narg, char **arg)
{
...
void DihedralListCA::write_restart(FILE *fp)
{
...
void DihedralListCA::read_restart(FILE *fp)
{
  allocate();
