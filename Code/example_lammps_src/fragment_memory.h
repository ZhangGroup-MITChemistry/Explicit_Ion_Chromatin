...
public:
  Fragment_Memory(int p, int pf, int l, double w, char *fname, bool vec_fm_flag=false, bool CustomGoPairsFlag=false, char *CaCaGoPairsFile=0, char *CaCbGoPairsFile=0);
  ~Fragment_Memory();
  ...
  int max(int, int);
  int error;
  enum Errors {ERR_NONE=0, ERR_FILE, ERR_ATOM_COUNT, ERR_RES, ERR_CALL, ERR_VFM_GLY, ERR_CUSTOM_GO};
...
private:
  ...
  // added by Bin for excluded go pairs 
  int customGoFlag;
};
