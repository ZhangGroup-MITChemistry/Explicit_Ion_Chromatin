  ...
  Fix(lmp, narg, arg)
{
  ...
  if(comm->me==0) efile = fopen("energy.log", "w");
  ...
  if(comm->me==0) fprintf(efile, "%s", eheader);
  ...
  read_custom_go_pairs_flag = water_exclusion_flag = 0; // added by Bin
  ...
  while (!in.eof()) {
	...
	in >> well_r_min[j] >> well_r_max[j] >> well_r_min_cut[j] >> well_r_max_cut[j] >> well_flag[j];
      ...
      // added by Bin: 08-12-2016
      // enable exclusion
      in >> nex_group;
      if (nex_group) {
        k_water_group = new double[nex_group];
        if (comm->me==0) print_log("Water exclusion flag is on\n");
        water_exclusion_flag = 1;
        ex1_group = ex2_group = ex1_bit = ex2_bit = NULL;
        memory->grow(ex1_group,nex_group,"neigh:ex1_group");
        memory->grow(ex2_group,nex_group,"neigh:ex2_group");
        
        char tmp[30], gs1[30], gs2[30];
        for (int ig=0; ig<nex_group; ++ig) {
          in >> tmp >> gs1 >> gs2 >> k_water_group[ig]; // group name
          ex1_group[ig] = group->find(gs1);
          ex2_group[ig] = group->find(gs2);
          if (ex1_group[ig] == -1 || ex2_group[ig] == -1)
            error->all(FLERR,"Invalid group ID in water exclusion command");
        }
      }
	...
	in >> h4prob[j];
      ...
      // added by bin; Read in amhgo groups; This way, we have more freedom in controlling the complex on which we will add go potential.
      int nchain, chainId, groupId;
      in >> nchain >> n_amhgo_group;
      amh_group = new int[nchain];
      for (int ic=0; ic<nchain; ++ic) {
          in >> chainId >> groupId; // group name
          amh_group[chainId-1] = groupId-1;
      }

      in >> read_custom_go_pairs_flag;
      if (read_custom_go_pairs_flag) {
          if (comm->me==0) print_log("Read Custom Go Pairs flag on\n");
  ...
  // looks like with partition flag, fprintf(screen) cannot be used.
  //fprintf(screen, "comm id=%d\n", comm->me);

  // Read sequance file; This has to be done before allocate. Otherwise nch will not be initialized // BinZhang
  ifstream ins(arg[6]);
  ...
  if (amh_go_flag) {
    ...
    if (m_amh_go->error==m_amh_go->ERR_CUSTOM_GO) error->all(FLERR,"Cannot open custom go pair files");
...
void FixBackbone::allocate()
{
  ...
  if (amh_go_flag) {
    ...
    amh_go_norm = new double[n_amhgo_group];
  }
...
Fragment_Memory **FixBackbone::read_mems(char *mems_file, int &n_mems)
{
  ...
  while ( fgets ( ln, sizeof ln, file ) != NULL ) {
    ...
    case FS_MEMS:
      ...
      //fprintf(stderr, "file: %s, %d, %d, %d, %f\n", str[0], tpos, fpos, len, weight);
...
// Added by Bin: 08-12-2016
// Remove water potential between pair of groups 
int FixBackbone::water_exclusion(int i, int j) const {

  int m, iatom, jatom;
  iatom = alpha_carbons[i];
  jatom = alpha_carbons[j];

  int *mask = atom->mask;

  // mask only work on atomic index,  but i and j are residue numbers.
  //fprintf(screen, "nex=%d, water=%d\n", nex_group, water_exclusion_flag);
  for (m = 0; m < nex_group; m++) {
    if (mask[iatom] & ex1_bit[m] && mask[jatom] & ex2_bit[m]) return m;
    if (mask[iatom] & ex2_bit[m] && mask[jatom] & ex1_bit[m]) return m;
  }
...
{	
  ...
  k_water_ij = k_water;
  if (water_exclusion_flag) {
      exclude_id = water_exclusion(i,j);
      if (exclude_id == -1) {
          k_water_ij = k_water;
      } else {
          k_water_ij = k_water_group[exclude_id];
      }
  }
  if (k_water_ij < delta) return;
  ...
  for (i_well=0;i_well<n_wells;++i_well) {
    ...
    energy[ET_WATER] += -epsilon*k_water_ij*sigma_gamma*well->theta(i, j, i_well);
		  
    force = epsilon*k_water_ij*sigma_gamma*well->prd_theta(i, j, i_well);
    ...
    if (!direct_contact) {
      ...
      well_prdHi_Hj = epsilon*k_water_ij*theta_gamma*well->prd_H(i)*well->H(j); 
      well_Hi_prdHj = epsilon*k_water_ij*theta_gamma*well->H(i)*well->prd_H(j);
...
void FixBackbone::compute_amhgo_normalization()
{
  ...
  int i, j, ich, res0, resn, iatom, jatom, ig, *amhgo_group_size;
  int ires_type, jres_type;
  double amhgo_gamma, rnative;
  double normi;
	
  amhgo_group_size = new int[n_amhgo_group];
  // Loop over chains
  for (ig=0; ig<n_amhgo_group; ++ig) {
    amh_go_norm[ig] = 0.0; //BinZhang
    amhgo_group_size[ig] = 0;
  }

  for (ich=0;ich<nch;++ich) {
      amhgo_group_size[amh_group[ich]] += ch_len[ich];
  }
  ...
  for (ich=0;ich<nch;++ich) {
	...
	//amh_go_norm[ich] += pow(fabs(normi), amh_go_p);
        amh_go_norm[amh_group[ich]] += pow(fabs(normi), amh_go_p); //BinZhang; do not use per chain normalization
      }
    }
    //amh_go_norm[ich] /= 8*resn[>;<]
  }	
  for (ig=0; ig<n_amhgo_group; ++ig) {
    amh_go_norm[ig] /= 8*amhgo_group_size[ig];     // BinZhang
    fprintf(stderr, "amhgo: %d, %12.6f,\n", amhgo_group_size[ig], amh_go_norm[ig]);
  }
  delete [] amhgo_group_size;
}
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if ( (mask[i]&groupbit || (mask[i]&group2bit && se[ires-1]!='G') ) && i<nlocal ) {
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        // allow inter molecular go
        if ( (mask[j]&groupbit || (mask[j]&group2bit && se[jres-1]!='G') ) && abs(ires-jres)>=amh_go_gamma->minSep() && (amh_group[imol-1]==amh_group[jmol-1]) ) {
          xj[0] = x[j][0];
          xj[1] = x[j][1];
          xj[2] = x[j][2];
          ...
          if (rnative<amh_go_rc) {
            //fprintf(screen, "i %d, j %d, r %12.6f\n", ires, jres, rnative);
	    dx[0] = xi[0] - xj[0];
            dx[1] = xi[1] - xj[1];
            dx[2] = xi[2] - xj[2];
      ...
      factor = -0.5*epsilon*k_amh_go*amh_go_p*pow(Ei, amh_go_p-1)/amh_go_norm[amh_group[imol-1]];
      for (k=0;k<nforces;k++) {
      ...
      E += -0.5*epsilon*k_amh_go*pow(Ei, amh_go_p)/amh_go_norm[amh_group[imol-1]];
    }
  }
...
void FixBackbone::table_fragment_memory(int i, int j)
{
  ...
  for (k=0;k<4;++k) {
    ...
    // Commenting this out shouldn't be a problem. We simply assume too far residues not interacting
    //else {
      //fprintf(stderr,  "step=%d, me=%d, r=%f\n", ntimestep, comm->me, r);
...
void FixBackbone::print_forces(int coord)
{
  ...
  int *res_tag = avec->residue; // BinZhang
  fprintf(dout, "fca = {");
  for (int i=0;i<nn;i++) {
    index = alpha_carbons[i];
    if (index!=-1) {
...
void FixBackbone::compute_backbone()
{
  ...
  for (i=0;i<nn;i++) {
	...
	compute_P_AP_potential(i, j);
      ...
      //if (water_flag && ( (i_chno!=j_chno && j_resno > i_resno ) || ( i_chno == j_chno && j_resno-i_resno>=contact_cutoff) ) && res_info[i]==LOCAL && (res_info[j]==LOCAL || res_info[j]==GHOST) && (!water_exclusion_flag || (water_exclusion_flag && !water_exclusion(i,j))) )
      if (water_flag && ( (i_chno!=j_chno && j_resno > i_resno ) || ( i_chno == j_chno && j_resno-i_resno>=contact_cutoff) ) && res_info[i]==LOCAL && (res_info[j]==LOCAL || res_info[j]==GHOST) )
      {
          compute_water_potential(i, j);
      }
