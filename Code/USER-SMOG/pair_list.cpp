...
static const char * const stylename[] = {
  "none", "harmonic", "morse", "lj126", "ca", NULL
};
...
void PairList::compute(int eflag, int vflag)
{
  ...
  for (int n=0; n < npairs; ++n) {
	...
	const double r = sqrt(rsq);
        const double dr = r - par.parm.ca.mu;
	const double r12  = mypow(rsq,6);
        const double sig2 = mypow(par.parm.ca.sigma, 2)*2.0;

        const double R = par.parm.ca.a / r12;
        const double dR = 12.0* R / rsq;

        const double G = exp(- dr*dr/sig2);
        const double dG = G* 2.0*dr/sig2/r;

        fpair = par.parm.ca.A*(dR - dR*G - R*dG - dG);

        //fprintf(screen,"Offset %f\n", par.offset);
	if (eflag_either)
	  epair = par.parm.ca.A*(R - R*G - G) - par.offset;
      }
...
void PairList::settings(int narg, char **arg)
{
  ...
  char *ptr;
  tagint id1, id2;
  int nharm=0, nmorse=0, nlj126=0, nca=0;
  ...
  while(fgets(line,1024,fp)) {
    ...
    } else if (strcmp(ptr,stylename[CA]) == 0) {
      // structure based model potential
      style[npairs] = CA;

      ptr = strtok(NULL," \t\n\r\f");
      if (!ptr)
	error->all(FLERR,"Incorrectly formatted structure-based pair parameters");
      par.parm.ca.A = force->numeric(FLERR,ptr);

      ptr = strtok(NULL," \t\n\r\f");
      if (!ptr)
	error->all(FLERR,"Incorrectly formatted structure-based pair parameters");
      par.parm.ca.mu = force->numeric(FLERR,ptr);

      ptr = strtok(NULL," \t\n\r\f");
      if (!ptr)
	error->all(FLERR,"Incorrectly formatted structure-based pair parameters");
      par.parm.ca.sigma = force->numeric(FLERR,ptr);
  ...
  if (comm->me == 0) {
    if (screen)
      fprintf(screen,"Read %d (%d/%d/%d/%d) interacting pairs from %s\n",
	      npairs, nharm, nmorse, nlj126, nca, arg[0]);
    if (logfile)
...
void PairList::init_style()
{
  ...
  if (offset_flag) {
	...
	par.offset = par.parm.morse.d0 * (dexp*dexp - 2.0*dexp);
        ...
        //const double r6inv = par.cutsq*par.cutsq*par.cutsq; // this does not seem correct at all
        const double r6inv = mypow(1.0/par.cutsq,3); 
	const double sig6  = mypow(par.parm.lj126.sigma,6);
	...
	const double r = sqrt(par.cutsq);
        const double dr = r - par.parm.ca.mu;
	const double r12  = mypow(par.cutsq,6);
        const double sig2 = mypow(par.parm.ca.sigma, 2)*2.0;

	par.offset = par.parm.ca.A*(1.0 + par.parm.ca.a/r12 )*( 1.0 - exp(- dr*dr/sig2) )- par.parm.ca.A ;
